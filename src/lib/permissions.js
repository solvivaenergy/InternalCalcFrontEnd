...
//   'cabling'            'Cabling component COGS (\u20B1)'      [Inventory tab]
...
export const ADMIN_SECTIONS = {
  ...
  cabling:           'AC/DC Cables, Conduits, Fittings, Panel Board & Other Devices (COGS, \u20B1)',
...
const ROLE_ADMIN_SECTIONS = {
  ...
  // Inventory-only editor. NOT part of the upstream role set — this deployment
  // added it as a Supabase role (backend migration
  // 20260731_add_inventory_role.sql), so it survives each upstream alignment.
...
// Labels follow this deployment's vocabulary, not upstream's ('Super Admin' /
// 'Product'), and carry the extra 'inventory' role.
const ROLE_LABELS = {
  edit:        'Management',
...
export const PARAM_KEY_TO_SECTION = {
  ...
  // Cabling
  // v3-216 — the pct keys (cablingTiers / cablingTiersThreePhase) are RETIRED:
  // absent from this map, so the server's section filter silently drops them
  // from any stale client's PUT and they persist in old blobs as dead data.
