...
// ─── v3-216 · CABLING COGS LADDER SEEDS (user-directed, Pat; D4/D5) ──────────
// The panel-array component groups are seeded as ABSOLUTE PESO COGS per anchor
// panel count, transcribed from Pat's revised seed sheet (this session) —
// BOTH tables entered explicitly; the v3-62 THREE_PHASE_CABLING_UPLIFT
// constant and deriveThreePhaseCablingTiers() are RETIRED with the pct model
// (three-phase is no longer derived from single-phase anywhere).
//
// Provenance notes against the sheet:
//   • Rows 1/8/10 are IDENTICAL by design (Pat leveled them to resolve the
//     old 8→10 dip) — the curve is flat from 1 to 10 panels.
//   • Rows 20/24 are identical — a deliberate flat segment.
//   • The sheet's displayed TOTAL column is off by ₱1 on some rows (Excel
//     shows rounded components but sums unrounded ones — SP@13 shows 48,058
//     vs the component sum 48,057; TP@13/16/20/24/32 similarly). Per Pat's
//     ruling (a) THE COMPONENTS GOVERN and the app derives totals, so the
//     derived totals are the component sums.
// MONOTONICITY: each row's total ≥ the prior row's — both seeds satisfy the
// peso floor at every step:
//   SP totals: 36,560 ×3 → 48,057 → 50,468 → 52,306 ×2 → 60,153
//   TP totals: 47,255 ×3 → 61,682 → 65,298 → 67,941 ×2 → 78,123
export const SINGLE_PHASE_CABLING_COGS_TIERS_DEFAULT = [
  { minPanels: 1,  dcCableCogs: 6623,  acCableCogs: 4549, conduitsCogs: 14246, panelBoardCogs: 11142 },
  { minPanels: 8,  dcCableCogs: 6623,  acCableCogs: 4549, conduitsCogs: 14246, panelBoardCogs: 11142 },
  { minPanels: 10, dcCableCogs: 6623,  acCableCogs: 4549, conduitsCogs: 14246, panelBoardCogs: 11142 },
  { minPanels: 13, dcCableCogs: 11376, acCableCogs: 5501, conduitsCogs: 15726, panelBoardCogs: 15454 },
  { minPanels: 16, dcCableCogs: 11376, acCableCogs: 5501, conduitsCogs: 15726, panelBoardCogs: 17865 },
  { minPanels: 20, dcCableCogs: 11376, acCableCogs: 6962, conduitsCogs: 16103, panelBoardCogs: 17865 },
  { minPanels: 24, dcCableCogs: 11376, acCableCogs: 6962, conduitsCogs: 16103, panelBoardCogs: 17865 },
  { minPanels: 32, dcCableCogs: 13897, acCableCogs: 9275, conduitsCogs: 17188, panelBoardCogs: 19793 },
];

export const THREE_PHASE_CABLING_COGS_TIERS_DEFAULT = [
  { minPanels: 1,  dcCableCogs: 6623,  acCableCogs: 6824,  conduitsCogs: 17095, panelBoardCogs: 16713 },
  { minPanels: 8,  dcCableCogs: 6623,  acCableCogs: 6824,  conduitsCogs: 17095, panelBoardCogs: 16713 },
  { minPanels: 10, dcCableCogs: 6623,  acCableCogs: 6824,  conduitsCogs: 17095, panelBoardCogs: 16713 },
  { minPanels: 13, dcCableCogs: 11376, acCableCogs: 8252,  conduitsCogs: 18872, panelBoardCogs: 23182 },
  { minPanels: 16, dcCableCogs: 11376, acCableCogs: 8252,  conduitsCogs: 18872, panelBoardCogs: 26798 },
  { minPanels: 20, dcCableCogs: 11376, acCableCogs: 10443, conduitsCogs: 19324, panelBoardCogs: 26798 },
  { minPanels: 24, dcCableCogs: 11376, acCableCogs: 10443, conduitsCogs: 19324, panelBoardCogs: 26798 },
  { minPanels: 32, dcCableCogs: 13897, acCableCogs: 13912, conduitsCogs: 20625, panelBoardCogs: 29689 },
];
...
export const ADMIN_PARAMS = {
  ...
  // The Battery Package (component K) EXITS the follow/fixed/otherwise scheme
  // below and rides its OWN GENLINV curve — over the order's TOTAL SELECTED
  // battery kWh (the Step 2 dropdown figure, D4), in EVERY order shape (with
  // or without panels/inverters, D1), covering ALL SIX package prices incl.
  // both labor variants (D2), ONE curve regardless of phase (D3). Anchors per
  // Pat (D5). Same closed form and clamping as the panels curve; a total at or
  // beyond the max anchor prices at the ceiling.
  // See batteryGrossMarginCurve() in calculations.js.
  //
  // ⚠ DEPLOYMENT DIVERGENCE FROM UPSTREAM v3-217 (deliberate, user-directed).
  // Upstream seeds 5/10/15 kWh → 21/24/29%. THIS deployment keeps the anchors
  // its own repo already carried under the pre-v3-209 key names
  // (grossMarginBatteryMinKwh/… → 5/15/30 kWh → 20/26/34%): the v3-209
  // IMPLEMENTATION is adopted, the VALUES are retained. migrateLegacyBatteryGm()
  // in paramsService.js carries a saved blob's old-key values onto these new
  // keys, so live pricing is unchanged by the rename.
  batteryGmMinKwh: 5,                    // kWh of the min-margin anchor
  batteryGmMidKwh: 15,                   // kWh of the mid-margin anchor (curvature)
  batteryGmMaxKwh: 30,                   // kWh of the max-margin anchor
  batteryGmMin: 0.20,                    // 20% — floor, at and below its kWh
  batteryGmMid: 0.26,                    // 26% — curvature
  batteryGmMax: 0.34,                    // 34% — ceiling, at and above its kWh

  // ─── v3-211 · FLAT-MODE OPTION PER CURVE (user-directed, Pat; D1–D3) ──────
  // Each of the three GM curves (single-phase panels, three-phase panels,
  // battery package) can be switched to FLAT: one margin at every capacity.
  // The flat value is its OWN editable parameter (D1 — deliberately NOT an
  // anchor reuse, so editing an anchor can never silently move a flat price);
  // seeds = each curve's Mid anchor (D2). Modes seed 'curve', so this release
  // moves NO prices and a legacy blob prices identically to v3-210. Anchors
  // persist untouched while flat — toggling back restores them exactly (D3).
  gmCurveModeSp: 'curve',                // 'curve' | 'flat'
  gmFlatSp: 0.22,
  gmCurveModeTp: 'curve',
  gmFlatTp: 0.22,
  batteryGmMode: 'curve',
  // D2's rule is "flat seed = that curve's Mid anchor"; applied to THIS
  // deployment's retained battery mid (0.26), not upstream's 0.24. Inert until
  // batteryGmMode is switched to 'flat'.
  batteryGmFlat: 0.26,
  ...
  // v3-209 — K is GONE from this table (battery rides its own kWh curve
  // above); normalizeComponentMargins drops a legacy blob's K row.
  componentMargins: {
  ...
  // v3-210 — sales commission allowance (user-directed, Pat; D1–D3). Charged
  // on the VAT-EXCLUSIVE direct purchase price (unlike the MDR's VAT-inclusive
  // base), universal across every COGS-derived price exactly like the MDR.
  // SEEDED LIVE AT 2% by decision D1 — a legacy blob missing the key takes
  // this on deploy, dropping retention 0.832 → 0.812 and lifting every DP
  // ~+2.46% at the 15% MDR. INTENTIONAL price movement, not drift.
  ...
  // ─── SINGLE-PHASE cabling COGS ladder (v3-216) ─────────────────────────────
  // ABSOLUTE PESO COGS per component group at each anchor panel count —
  // decoupled from panel COGS (Pat's directive: a panel-COGS change must not
  // force Anjon to re-tune this table). Piecewise-linear between anchors;
  // per-panel rate holds flat above the last anchor (D1/D2). Applies when
  // Step 1A service type = Single-phase.
  cablingCogsTiers: SINGLE_PHASE_CABLING_COGS_TIERS_DEFAULT.map(t => ({ ...t })),

  // ─── THREE-PHASE cabling COGS ladder (v3-216) ──────────────────────────────
  // Same structure; applies when Step 1A service type = 3-phase. Seeded
  // EXPLICITLY from Pat's sheet — no longer derived from single-phase.
  // Not in the Excel reference workbook — Excel mirror update deferred.
  cablingCogsTiersThreePhase: THREE_PHASE_CABLING_COGS_TIERS_DEFAULT.map(t => ({ ...t })),
